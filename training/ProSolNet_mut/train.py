...
class Datasets(Dataset):
    def __init__(self, keys, feat_list, xyz_list, surf_feat_list, surf_res, lm_list,
                 wt_feat_list, wt_xyz_list, wt_surf_feat_list, wt_surf_res, wt_lm_list, labels):
        self.keys = keys
    ...
    def __getitem__(self, idx):
        return self.keys[idx], self.feat_list[idx], self.xyz_list[idx], self.surf_feat_list[idx], \
            self.surf_res[idx], self.lm_list[idx], self.wt_feat_list[idx], self.wt_xyz_list[idx], \
            self.wt_surf_feat_list[idx], self.wt_surf_res[idx], self.wt_lm_list[idx], self.labels[idx].unsqueeze(0)
    ...
    def data_collate(self, data):
        keys, feats, xyz, surf_feat, surf_res, lm_feats, wt_feats, wt_xyz, wt_surf_feat, wt_surf_res, wt_lm_feats, labels = zip(*data)
        ...
        return {"keys": keys, "weights": weights, "feats": feats, "xyz": xyz_tensor, "surf_feats": final_surf_feat,
                "surf_res": final_surf_res, "lm_feat": lm_feats, "res_batch": res_batch, "batch": batch, "wt_feats": wt_feats,
                "wt_xyz": wt_xyz_tensor, "wt_surf_feats": wt_final_surf_feat, "wt_surf_res": wt_final_surf_res, "wt_lm_feat": wt_lm_feats,
                "wt_res_batch": wt_res_batch, "wt_batch": wt_batch, "labels": labels}

class ProteinDataset():
    def __init__(self, struc_fpath, surf_fpath, lm_fpath, split_fpath, label_fpath, batch_size):
    ...
    def process_feats(self, keys, reverse, shuffle):
        ...
        dataset = Datasets(final_keys, res_feat_list, xyz_list, surf_feat_list, surf_res_index, lm_feat_list,
                           wt_res_feat_list, wt_xyz_list, wt_surf_feat_list, wt_surf_res_index, wt_lm_feat_list, label_tensor)
    ...
    def get_dataloader(self, idx):
        ...
        _, train_dataloader = self.process_feats(train_keys, True, True)
        final_valid_keys, valid_dataloader = self.process_feats(valid_keys, False, False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-label_fpath", type=str, default="all_label.npy",
    ...
    patience = 10
    min_delta = 0.001
    data = ProteinDataset(struc_fpath, surf_fpath, lm_fpath, split_fpath, label_fpath, batch_size)

    all_valid_pred_labels = []
    all_valid_keys = []
    for fold in range(1, 11):
        print(f"fold: {fold}/10")
        train_dataloader, valid_keys, valid_dataloader = data.get_dataloader(fold)
    ...
    valid_eval_df = pd.DataFrame(valid_eval.reshape(1, -1), index=["valid"], columns=["ACC", "norm-ACC", "macro-f1"])
    valid_eval_df.to_csv("results.csv")

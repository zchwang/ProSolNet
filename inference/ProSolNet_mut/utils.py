...
def out_results(values, file_):
    columns = ["ACC", "norm-ACC", "macro_f1", "loss"]
    df = pd.DataFrame(values.reshape(2, -1), index=["valid", "test"], columns=columns)
...
def write_logfile(epoch, record_data, logfile):
    ...
    columns = ["epoch", "T_ACC", "T_norm-ACC", "T_macro-f1", "T_loss", "V_ACC", "V_norm-ACC", "V_macro-f1", "V_loss",
               "t_ACC", "t_norm-ACC", "t_macro-f1", "t_loss"]
    df = pd.DataFrame(values, index=index, columns=columns)
    df.to_csv(logfile, float_format="%.4f")

"""
def recover_pred(pred_tensor):
    pred_1 = (pred_tensor >= -1.5) * (pred_tensor < -0.5) * -1
    pred_2 = (pred_tensor >= -0.5) * (pred_tensor < 0.5) * 0
    pred_3 = (pred_tensor >= 0.5) * (pred_tensor <= 1.5) * 1
...
def run_a_training_epoch(model, dataloader, optimizer, device):
    ...
    for step, data in enumerate(dataloader):
        #{"keys": keys, "seq_feats": seq_feats, "feats": feats, "xyz": xyz_tensor, "surf_feats": final_surf_feat, 
        #        "surf_res": final_surf_res, "res_batch": res_batch, "batch": batch, "wt_seq_feats": wt_seq_feats, "wt_feats": wt_feats,
        #        "wt_xyz": wt_xyz_tensor, "wt_surf_feats": wt_final_surf_feat, "wt_surf_res": wt_final_surf_res, "wt_res_batch": wt_res_batch,
        #        "wt_batch": wt_batch, "labels": labels}
        keys, weights, seq_feats, feats, xyz, surf_feats, surf_res, lm_feats, res_batch, batch, wt_seq_feats, wt_feats, \
        ...
        wt_seq_feats = wt_seq_feats.to(device)
        ...
        if len(labels) == 1:
            continue
        _, pred_proba = model(seq_feats, feats, xyz, surf_feats, surf_res, lm_feats, res_batch, batch,
                              wt_seq_feats, wt_feats, wt_xyz, wt_surf_feats, wt_surf_res, wt_lm_feats, wt_res_batch, wt_batch)
        #loss = nn.MSELoss()(pred_proba.ravel(), label.ravel())
